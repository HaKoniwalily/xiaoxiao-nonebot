...
async def handle_xuanshang_matcher(bot: Bot, event: GroupMessageEvent):
    ...
    if xuanshang_list:
        ...
        if best_xuanshang_info:
            await bot.send_group_msg(group_id=config["group_id"], message=MessageSegment.at(xx_id) + " 悬赏令接取" + best_xuanshang_info["序号"])
    return
    ...
    for line in fangshi_data:
        ...
        if match:
            ...
            if item_name in required_items:
                try:
                    prices_list = [int(price) for price in price_str.split("/")]

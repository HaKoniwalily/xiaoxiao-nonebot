...
# 启用炼丹辅助群号
allowed_group_ids = [761433933,769376338]
config_group_id = CONFIG.get("group_id")
if config_group_id:
    allowed_group_ids.append(int(config_group_id))
...
async def at_me(bot: Bot, event: GroupMessageEvent) -> bool:
    ...
    return self_id in str(event) or event.group_id in allowed_group_ids
...
async def liandan(event: GroupMessageEvent):
    ...
    if pill_path.exists():
        config.read(pill_path, encoding="utf-8-sig")
    if pill_name not in config:
        config.add_section(pill_name)
    if recipe not in config.options(pill_name):
        config.set(pill_name, recipe, None)
    with open(pill_path, "w", encoding="utf-8-sig") as f:
        config.write(f)
        print(f"丹方已保存到{pill_path}")
    # 返回结果
    price, aftertax_price = shangjia_price(pill_name)

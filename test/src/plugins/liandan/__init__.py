...
xx_id = "3889001741"
allowed_group_ids = [761433933, 769376338, 634253191]
...
# 炼丹指令
liandan = on_regex(r"^炼丹$", priority=1, block=True)
...
def is_bag(event: Event) -> bool:
    return event.get_user_id() == xx_id and "丹" not in event.get_plaintext() and event.group_id in allowed_group_ids

liandan_matcher = on_message(rule=is_bag, priority=2, block=True)
